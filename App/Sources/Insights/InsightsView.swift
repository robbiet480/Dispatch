...
struct InsightsView: View {
    ...
    /// Memoized-viz pattern (HomeView.visualizationTaskID): count + newest
    /// date + identity fingerprint. Unrelated re-renders don't refire the
    /// compute; edits/deletes/backfills do.
    ...
    private var explainer: some View {
        Text("Patterns across all \(reports.filter { !$0.isDraft }.count) of your reports. These are tendencies, not causes — small or noisy differences stay hidden.")
            .font(.footnote)
    ...
    private var emptyState: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("No insights yet")
                .font(.headline)
                .foregroundStyle(.white)
            Text("Insights look for steady patterns across your reports — like which answers tend to come with more steps or a better mood. They need around two weeks of regular reports before anything is solid enough to show. Keep filing and check back.")
                .font(.subheadline)
