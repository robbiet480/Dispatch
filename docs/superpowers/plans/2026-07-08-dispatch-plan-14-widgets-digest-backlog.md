...
- **Medications capture is IN (user decision, amendment; superseded by T5 revision):** the original day-one crash came from putting the medication dose type into the BULK `requestAuthorization` read set (uncatchable NSInvalidArgumentException on device). The implementation must determine the correct authorization path EMPIRICALLY from the iOS 26 SDK headers (HealthKit medication APIs — check for per-object read authorization à la vision prescriptions, `HKUserAnnotatedMedication`/medication dose event APIs and their documented auth requirements) — never re-adding the type to the bulk set. As implemented (T5 second revision, user decision): the `healthMedications` toggle ships DEFAULT ON like the other health sensors, with NO separate opt-in — the header-verified per-object authorization call (`requestPerObjectReadAuthorization(for: .userAnnotatedMedicationType())`) runs as its own sequenced step inside the standard permission cascade (onboarding and Settings → "Request Sensor Access"). The earlier default-OFF/explicit-"Request Medications Access" design described in prior drafts is superseded. Hardware verification (fresh-install onboarding crash regression) remains the repo owner's.
...
- **T5:** medications capture via the header-verified iOS 26 path: `requestPerObjectReadAuthorization(for: .userAnnotatedMedicationType())` as its own sequenced cascade step after bulk HealthKit (never in the bulk read set — exclusion comment references the day-one device crash); HKUserAnnotatedMedicationQuery resolves concept → nickname/displayText; today's taken/skipped dose events map to `medication.<status>.<name>` readings (kit MedicationReading, pure + tested); checklist "N MEDICATIONS LOGGED" row; detail "Name · dose · status" rows; healthMedications toggle default ON. Done = suites green + archive clean; hardware verification (incl. fresh-install onboarding crash regression) is the repo owner's — device script in .superpowers/sdd/plan-14-report.md.
