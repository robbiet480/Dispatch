...
- **Payload encryption (optional, on top of signing — Robbie 2026-07-09):** an "Encrypt payload" toggle (requires a secret). When on, the JSON payload is encrypted with AES-256-GCM, key = HKDF-SHA256(secret, salt: `io.robbie.Dispatch.webhook`, info: `payload-encryption`, 32 bytes), and the body becomes the envelope `{"encrypted": true, "algorithm": "aes-256-gcm", "data": "<base64 of AES.GCM.SealedBox.combined>"}`. Encrypt-then-MAC: the HMAC header signs the envelope bytes as sent. Applies to all events incl. test and bulk. Kit `WebhookCrypto` with tests verified against a python `cryptography` reference (session crypto rule); a receiver decrypt recipe (Python snippet) ships in `docs/webhooks.md` alongside the payload/signature docs. **Export-compliance check (do, don't assume):** fetch Apple's current ITSAppUsesNonExemptEncryption/exemption documentation and verify the existing `NO` declaration remains valid for OS-provided standard-algorithm crypto (CryptoKit AES-GCM); document the verdict in the report and update docs/app-store/review-readiness.md's encryption row.
...
- **Delivery + retry (revised per Robbie 2026-07-09):** enqueue-and-drain. Every successful report save (in-app survey, notification quick-answer, widget quick-answer via its existing marker pattern, backfill, intents) enqueues the report's uniqueIdentifier in app-group defaults. The app drains: immediately after in-app saves, on foreground, and after the widget-marker drain. Success = HTTP 2xx (the user said "200"; whole 2xx family counts, documented). Per attempt: 15s timeout; failure → retained with attempt count, retried at subsequent drain opportunities, **capped at 3 attempts**. On the 3rd failure: a LOCAL NOTIFICATION tells the user delivery failed ("Webhook delivery failed for your <time> report — check the URL in Settings"), identifier `webhook-failed-<reportID>` (joins the standard removal-batch prefix discipline), and the report drops from the queue (status row shows the failure). Deliberately NO background URLSession in v1 (drain-on-foreground is honest and simple; noted as future work).
- **Send All Reports (one-time bulk):** a button in the webhook settings ("Send All Reports…") with a mode choice presented at tap time: **individual** (every report enqueued through the normal queue as `report.created` events, delivered oldest-first with the same retry/notification semantics) or **single payload** (`{"event": "report.bulk", "schemaVersion": 2, "reports": [<v2 report objects, oldest-first>]}`, one POST, 60s timeout, one retry offer via alert on failure — the bulk payload does NOT enter the retry queue). Confirmation states the count before sending. Identifier `webhook-send-all`.
