...
struct BackupWriterTests {
    ...
    func createsDirectoryOnFirstWrite() throws {
        ...
        let filename = BackupRotation.backupFilename(for: Date())
        let count = try BackupWriter.writeAndRotate(
            data: Data("{}".utf8), filename: filename, in: directory)
    ...
    func prunesExpiredBackupsOnly() throws {
        ...
        for daysAgo in 1...3 {
            ...
            let name = BackupRotation.backupFilename(for: date)
        ...
        let count = try BackupWriter.writeAndRotate(
            data: Data("{}".utf8), filename: BackupRotation.backupFilename(for: now),
            in: directory, keep: 2)
