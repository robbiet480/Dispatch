...
/// Mac twin of the iOS ReportDetailView (audited for dual membership first:
/// the iOS file leans on UIApplication for the media deep link plus
/// iOS-only navigation-bar modifiers, so per the shared-file discipline it
/// stays untouched and this twin renders the same kit-backed sections).
struct MacReportDetailView: View {
    let report: Report
    /// Returns to the dashboard (clears the sidebar selection).
    var onBack: () -> Void = {}
        ...
        .toolbar {
            ToolbarItem(placement: .navigation) {
                Button {
                    ...
                    Label("Back to Dashboard", systemImage: "chevron.backward")
                }
    ...
    /// Tappable when `MediaDeepLink` resolves a URL. macOS has no
    /// canOpenURL; `NSWorkspace` answers whether anything handles the
    /// spotify: scheme (the Spotify desktop app registers it).
    @ViewBuilder
    private func mediaRow(_ media: MediaSample) -> some View {
        let spotifyInstalled = URL(string: "spotify:").flatMap {
    ...
    private var backdatedChip: some View {
        Text("BACKDATED")
            ...
            .accessibilityLabel("Backdated report")
